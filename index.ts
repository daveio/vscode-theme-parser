...
import { logError, logInfo, registerHandlebarsHelpers } from "./src/utils";
import { logError, registerHandlebarsHelpers } from "./src/utils";
...
program.action(() => {
	if (process.argv.length <= 2) {
		...
		console.log(
			chalk.green(
				"  vscode-theme-parser parse --input <input-file> --output <output-file>",
			),
		);
		console.log(chalk.green("  vscode-theme-parser about"));
		console.log(
			"\nRun " +
				chalk.cyan("vscode-theme-parser --help") +
